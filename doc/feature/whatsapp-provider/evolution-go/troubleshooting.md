...
| Sync contact info não muda avatar | Job async (`ContactEnrichmentJob` + `force: true`). Esperar alguns segundos e recarregar; se `/user/info` vier rate-limited, `enriched_at` não carimba — tentar de novo mais tarde. Se Go continuar em `ReadTimeout`, ver [avatar-failures-report.md](./avatar-failures-report.md) |
