...
| Refresh de contatos não atualiza foto | Preferir `PictureURL`/`PictureID` de `/user/info` (evita `/user/avatar` extra). Avatar pode ser URL ou base64. Query: LID → JID → dígitos (BR sem forçar `phone@s.whatsapp.net`). Rate-limit não marca cooldown longo. Refresh bulk: stagger 3s. `/user/avatar` + `/user/info` non-retryable (timeout avatar 12s). `evolution_go_avatar_attempted_at` 6h; `force: true` / botão Refresh tenta de novo |
...
| Sync contact info não muda avatar | Job async (`ContactEnrichmentJob` + `force: true`). Esperar alguns segundos e recarregar; se `/user/info` vier rate-limited, `enriched_at` não carimba — tentar de novo mais tarde |
