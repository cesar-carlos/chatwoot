...
| Delete/edit no celular não reflete no CW | Delete: corrigido jul/2026 (`fromMe` + protocol antes do echo). **Edit:** se o webhook só trouxer `secretEncryptedMessage` (sem texto), o CW **não** atualiza — limitação do Go ([#62](https://github.com/evolution-foundation/evolution-go/issues/62), [#92](https://github.com/evolution-foundation/evolution-go/issues/92)); verificar log `skipped encrypted edit envelope` |
...
| Edit do cliente cria `[Unsupported message type]` | Go envia `Info.Edit=1` + `secretEncryptedMessage` (sem plaintext/`protocolMessage`); job detecta `encrypted_edit` e descarta o envelope — evita placeholder. Conteúdo só atualiza se vier plaintext (`protocolMessage` / `MESSAGES_EDITED` / `editedMessage`) |
| Cliente edita no WA e o texto no CW não muda (sem unsupported) | Esperado quando o Go não decripta o edit. Confirmar payload: `Info.Edit`/`IsEdit` + `secretEncryptedMessage` sem `editedMessage.conversation`. Não há workaround no fork até o Go entregar plaintext. Comparar com Evolution Node (`MESSAGES_EDITED` costuma trazer `editedMessage`) |
...
| Edit no CW falha / não chega no WA | `POST …/evolution_go_edit` → sync WA **primeiro** (`EditSyncService raise_errors: true`) → só então persiste no CW. Erro da API aparece no modal. Checar log `[EVOLUTION_GO] edit sync` / janela ~15 min / `ChatJid` |
