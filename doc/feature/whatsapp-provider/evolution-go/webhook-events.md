...
| LID 1:1 + alt | `xxx@lid` + alt de telefone | `wa_id` = dígitos do alt (adapter prefere PN sobre `@lid` em SenderAlt/RecipientAlt). `evolution_go_remote_jid` = `@lid` (addressing — **não** o PN reescrito) |
| LID 1:1 sem alt | `xxx@lid` sem PN | **Não dropa.** `wa_id`/`from` = JID `@lid` completo (igual grupos `@g.us`). Não inventar E.164 dos dígitos LID |
