...
- [ ] Dois inboxes Evolution Go da mesma conta: envio em A cria bolha inbound em B **com a mesma** `source_id` (lock Redis **não** é global — [§38](./decisions.md))
- [ ] Unique ID vazio na sidebar **não** impede envio se `evolution_go_remote_jid` for `@lid`
