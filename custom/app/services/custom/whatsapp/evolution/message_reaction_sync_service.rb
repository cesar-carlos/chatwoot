...
class Custom::Whatsapp::Evolution::MessageReactionSyncService
  ...
  def perform
    ...
    if message.blank?
      Rails.logger.info(
        "[EVOLUTION] inbound_reaction_skipped channel=#{channel.id} target=#{key[:id]}"
      )
      return false
    end
  ...
  def resolve_contact(jid_or_phone)
    ...
    phone = Custom::Whatsapp::Evolution::JidResolver.new(channel.provider_config || {})
