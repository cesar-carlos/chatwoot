...
module Custom::Whatsapp::IncomingMessageIdentifierHelper
  ...
  def set_contact_from_message
    super
    enqueue_evolution_contact_enrichment if evolution_channel? && @contact.present? && !evolution_group_inbound?
  end
  ...
  def evolution_group_inbound?
    evolution_channel? &&
      Custom::Whatsapp::Evolution::GroupContactService.group_jid?(evolution_remote_jid_from_message)
  end
  ...
  def evolution_participant_jid_from_message
    message = messages_data&.first
    return if message.blank?
  ...
  def sync_group_participant!
    participant_jid = evolution_participant_jid_from_message
